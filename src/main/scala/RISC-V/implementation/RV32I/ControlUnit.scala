  ...
  when(was_stalled === STALL_REASON.EXECUTION_UNIT) {
    ...
    switch(RISCV_TYPE.getOP(io_ctrl.instr_type)) {
      ...
      is (RISCV_OP.LOAD) {
        ...
        // set data_be based on funct3 for byte, halfword, word loads
        io_ctrl.data_be := MuxLookup(
          RISCV_TYPE.getFunct3(io_ctrl.instr_type),
          "b0000".U(4.W)  // Default to no bytes enabled for safety
          )(
            Seq(
              RISCV_FUNCT3.F000 -> "b0001".U,
              RISCV_FUNCT3.F001 -> "b0011".U,
              RISCV_FUNCT3.F010 -> "b1111".U,
              RISCV_FUNCT3.F100 -> "b0001".U,
              RISCV_FUNCT3.F101 -> "b0011".U
            )
          )
      }
    }
