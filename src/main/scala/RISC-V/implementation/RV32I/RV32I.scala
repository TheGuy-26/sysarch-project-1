...
class RV32I(
  ...
  // exercise 2.3
    is(REG_WRITE_SEL.MEM_OUT_SIGN_EXTENDED) {
      ...
      io_reg.reg_write_data := MuxLookup(funct3.asUInt, io_data.data_rdata)(
        Seq(
          // Sign-extend byte (LB)
          RISCV_FUNCT3.F000.asUInt -> Cat(Fill(24, io_data.data_rdata(7)), io_data.data_rdata(7, 0)),
          // Sign-extend halfword (LH)
          RISCV_FUNCT3.F001.asUInt -> Cat(Fill(16, io_data.data_rdata(15)), io_data.data_rdata(15, 0)),
          // Full word (LW)
          RISCV_FUNCT3.F010.asUInt -> io_data.data_rdata
        )
      )
    }
    ...
    is(REG_WRITE_SEL.MEM_OUT_ZERO_EXTENDED) {
      ...
      io_reg.reg_write_data := MuxLookup(funct3.asUInt, io_data.data_rdata)(
        Seq(
          // Zero-extend byte (LBU)
          RISCV_FUNCT3.F100.asUInt -> Cat(0.U(24.W), io_data.data_rdata(7, 0)),
          // Zero-extend halfword (LHU)
          RISCV_FUNCT3.F101.asUInt -> Cat(0.U(16.W), io_data.data_rdata(15, 0))
        ) 
      )
    }
  }
