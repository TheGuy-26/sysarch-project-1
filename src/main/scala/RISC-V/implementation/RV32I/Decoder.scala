...
class Decoder extends AbstractDecoder {
  ...
  switch(opcode) {
    ...
    is(RISCV_OP.JAL) {
      io_decoder.rd := RD
      io_decoder.imm := Fill(12, io_decoder.instr(31)) ## io_decoder.instr(19,12) ## io_decoder.instr(20) ## io_decoder.instr(30, 21) ## 0.U
    }
    is(RISCV_OP.OP) {
      io_decoder.rs1 := RS1
      io_decoder.rs2 := RS2
    ...
    is(RISCV_OP.LOAD) {
      io_decoder.rs1 := RS1
      io_decoder.rs2 := 0.U // we don't need rs2 in load instructions
      io_decoder.rd := RD
      io_decoder.imm := Fill(20, io_decoder.instr(31)) ## io_decoder.instr(31, 20)
    }
  }
